...
<#
.SYNOPSIS
    Active Directory data collector for Access Review Toolbox.
    Dumps users, computers, groups, and OUs to JSON files in $DumpPath.
#>
param(
    [Parameter(Mandatory)][string]$DumpPath,
    [string]$ConfigPath   # not used for AD (uses current user's session)
)
...
function Write-Manifest {
    ...
    $manifest | ConvertTo-Json -Depth 10 | Out-File (Join-Path $DumpPath 'manifest.json') -Encoding UTF8
}
...
try {
    ...
    $errors.Add("Users collection failed: $($_.Exception.Message)") | Out-Null
...
try {
    ...
    $errors.Add("Computers collection failed: $($_.Exception.Message)") | Out-Null
...
try {
    ...
    $errors.Add("Groups collection failed: $($_.Exception.Message)") | Out-Null
...
try {
    $ous = Get-ADOrganizationalUnit -Filter * -Properties Description,WhenCreated,WhenChanged | ForEach-Object {
        $ou = $_
        # Count direct child objects
        $childCount = 0
        try {
            $childCount = (Get-ADObject -SearchBase $ou.DistinguishedName -SearchScope OneLevel -Filter *).Count
        } catch {}
        [ordered]@{
            Name              = $ou.Name
    ...
    $errors.Add("OUs collection failed: $($_.Exception.Message)") | Out-Null
...
$finalStatus = if ($errors.Count -eq 0) { 'complete' } else { 'complete_with_errors' }
